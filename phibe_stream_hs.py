# "high speed" version, first channel only at twice the speed
...
info_phibe = StreamInfo('phi','phi',1,samplingrate,'float32','conphyture-phibe')
outlet_phibe = StreamOutlet(info_phibe, max_buffered=1)

class Board(Peripheral):
    def __init__(self, addr):
        # list of channels -- in HS version more like number of values for one channel
        self.nbChans = 2
        self.buffer = ''
        # current position
        self.head = 0
...
if __name__=="__main__":
    try:
        ...
        while True:
            # only one channels over xxx values
            board.waitForNotifications((1. * board.nbChans)/samplingrate)
            # ugly way to stream and free board current buffer
            for s in board.samples:
                print "push sample: ", s.id, "values: [",
                # each value pushed to one channel
                for v in s.channel_data:
                  print v,
                  outlet_phibe.push_sample([v])
                print "]"
            board.samples = []
